...
# Add token to coin flip game whitelist
# Usage: ./add_token.sh <network> <token_type> <rpc_url> <gas_budget> [ledger_mode] [ledger_address] [gas_object_id]
...
if [ -z "$NETWORK" ] || [ -z "$TOKEN_TYPE" ] || [ -z "$RPC_URL" ] || [ -z "$GAS_BUDGET" ]; then
    ...
    echo -e "${YELLOW}Example: $0 testnet 0x2::sui::SUI https://fullnode.testnet.sui.io:443 200000000${NC}"
...
if [ "$LEDGER_MODE" = "true" ]; then
    ...
    show_ledger_instructions "Add Token: $TOKEN_TYPE to Whitelist" "$TX_OUTPUT"
    exit 0
else
...
echo -e "${GREEN}Games can now be created with this token${NC}"
echo -e "${GREEN}========================================${NC}" 