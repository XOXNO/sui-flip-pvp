...
# Usage: ./set_fee.sh <network> <fee_bps> <rpc_url> <gas_budget> [ledger_mode] [ledger_address] [gas_object_id]
...
if [ -z "$NETWORK" ] || [ -z "$FEE_BPS" ] || [ -z "$RPC_URL" ] || [ -z "$GAS_BUDGET" ]; then
    echo -e "${RED}Usage: $0 <network> <fee_bps> <rpc_url> <gas_budget> [ledger_mode] [ledger_address] [gas_object_id]${NC}"
    exit 1
fi
...
if [ "$LEDGER_MODE" = "true" ]; then
    ...
    show_ledger_instructions "Set Fee to $FEE_BPS bps ($(echo "scale=2; $FEE_BPS/100" | bc)%)" "$TX_OUTPUT"
    exit 0
else
